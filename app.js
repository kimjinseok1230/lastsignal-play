import {specialState,refill,reserve,refund,settle,configureSpecial,REFILL_MS} from './special-shift.js?v=cat29';
import {COSMETICS,hasCosmetic,equipCosmetic,cosmeticCard,applyLobbyOutfit,mountThemePreviews} from './cosmetics.js?v=cat29';
import {SYNERGIES,synergyHints,nextGoals} from './build-systems.js?v=cat29';
import {endingScene} from './ending-scene.js?v=2';
import {bossProfile} from './cat-bosses.js?v=cat29';
import {missionUnlocks,buyCat,buyTicket,drawCat,DRAW_COST,TICKET_COST,PASSIVES,ACTIVE_SKILLS} from './roster.js?v=cat29';
import {bindStick} from './aim.js?v=cat29';
import {SignalGame as Game} from './cat-game.js?v=cat29';
import {Sound} from './audio.js?v=cat29';
import {CLASSES,UPGRADES,EVOLUTIONS,BASE_UPGRADES,DURATION,WEAPONS,PROTOCOLS,createMeta,normalizeMeta,formatTime,xpRequired} from './data.js?v=cat29';
...
function showWardrobe(){
 ...
 openModal(`${closeButton}<div class="eyebrow">야간조 드레스룸 · ${count} / ${COSMETICS.length}</div><h2 id="modal-title">오늘은 어떤 모습으로 출근할까요?</h2><p class="modal-intro">공격력·체력 변화 없음. 의상은 로비와 전투, 이름표는 로비, 처치 효과는 적을 잡을 때 적용됩니다. 모든 의상과 효과는 전투 미리보기에서 체험할 수 있어요.</p><div class="wardrobe-pass"><strong>무료 출근 보상</strong><span>1회 근무 · 민트 스카프 ${meta.runs>=1?'✓':''}</span><span>3회 근무 · 캣닢 꽃잎 ${meta.runs>=3?'✓':''}</span><span>첫 퇴근 성공 · 이름표 ${meta.wins>=1?'✓':''}</span></div><div class="starter-preview"><div><small>판매 준비 중 · 가격 초안 3,900원</small><h3>첫 출근팩 · 꾸미기 3종</h3><p>야간조 유니폼 + 말랑 발바닥 처치 효과 + 복숭아빛 이름표</p><a class="secondary-button" target="_blank" rel="noopener" href="./test-room.html?v=cat29&look=starter&cat=${selectedClass}">첫 출근팩 전투 미리보기 ↗</a></div><span aria-hidden="true">🐾</span></div><div class="wardrobe-tabs" aria-label="꾸미기 분류">${[['all','전체'],['outfit','의상'],['effect','처치 효과'],['frame','이름표']].map(([id,name])=>`<button class="secondary-button" data-look-tab="${id}" aria-pressed="${wardrobeTab===id}">${name}</button>`).join('')}</div><div class="cosmetic-grid">${COSMETICS.filter(c=>wardrobeTab==='all'||c.slot===wardrobeTab).map(c=>cosmeticCard(c,meta,selectedClass)).join('')}</div><p class="upgrade-note">무료 보상은 완료한 근무 기록으로 해금됩니다. 판매 준비 중 상품은 미리보기 전용이며 현재 결제할 수 없습니다. 착용 기록은 이 브라우저에 저장됩니다.</p>`,hideModal);mountThemePreviews($('modal'));
}
let adPending=false;
...
function menuRefresh(){
  if(game.state==='menu')game.classId=selectedClass;game.cosmetics={...meta.cosmetics};$('lobby-cat').dataset.outfit=meta.cosmetics.outfit;document.querySelector('.lobby-cat-label').dataset.frame=meta.cosmetics.frame;
  ...
  applyLobbyOutfit($('lobby-cat'),partner.id,meta.cosmetics.outfit);$('lobby-cat').alt=partner.name;
  $('lobby-name').textContent=partner.name+(meta.cosmetics.outfit==='scarf'?' · 민트 스카프':'');$('lobby-weapon').textContent=partner.tag;
