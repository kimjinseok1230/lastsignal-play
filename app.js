import {missionUnlocks,buyCat,buyTicket,drawCat,DRAW_COST,TICKET_COST,PASSIVES} from './roster.js?v=cat8';
import {bindStick} from './aim.js?v=cat8';
import {SignalGame as Game} from './cat-game.js?v=cat8';
import {Sound} from './audio.js?v=cat8';
import {CLASSES,UPGRADES,EVOLUTIONS,BASE_UPGRADES,DURATION,WEAPONS,PROTOCOLS,createMeta,normalizeMeta,formatTime,xpRequired} from './data.js?v=cat8';
...
function updateHUD(force=false){
  ...
  $('dash-status').textContent=p.dashCd>0?p.dashCd.toFixed(1)+'s':'준비 완료';$('pulse-status').textContent=p.pulseCd>0?Math.ceil(p.pulseCd)+'s':'준비 완료';$('dash-fill').style.width=(1-p.dashCd/game.stats.dashCd)*100+'%';$('pulse-fill').style.width=(1-p.pulseCd/game.stats.pulseCd)*100+'%';$('dash-btn').setAttribute('aria-disabled',String(p.dashCd>0));$('pulse-btn').setAttribute('aria-disabled',String(p.pulseCd>0));
