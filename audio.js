...
export class Sound {
 ...
 noise(duration=.06,volume=.04,cutoff=3000,delay=0,bus='effects'){if(!this.ready()||this.voices>=72)return;const t=this.ctx.currentTime+Math.max(0,delay),n=this.ctx.createBufferSource(),g=this.ctx.createGain(),f=this.ctx.createBiquadFilter();n.buffer=this.noiseBuffer;f.type='highpass';f.frequency.value=cutoff;g.gain.setValueAtTime(volume,t);g.gain.exponentialRampToValueAtTime(.0001,t+duration);n.connect(f);f.connect(g);g.connect(this[bus]);this.voices++;n.onended=()=>{n.disconnect();f.disconnect();g.disconnect();this.voices--;};n.start(t);n.stop(t+duration);}
 ...
 play(name){if(!this.ready())return;
  ...
  else if(name==='explode'||name==='mine'){this.tone(110,.25,'triangle',.18,28);this.noise(.18,.1,500);}
  else if(name==='arc'||name==='laser'){if(this.allow(name,.12)){this.tone(1100,.12,'sawtooth',.035,150);this.noise(.045,.04,2800);}}
  else if(name==='rail'){this.tone(230,.22,'triangle',.12,55);this.noise(.1,.08,2200);}
