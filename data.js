import {EXTRA_CATS,collectionMeta} from './roster.js?v=cat12';
...
export function createMeta(){return {version:VERSION,credits:0,base:{},runs:0,wins:0,bestTime:0,bestKills:0,totalKills:0,history:[],collection:collectionMeta(),settings:{sound:true,volume:.36,musicVolume:.55,effectsVolume:.8,shake:true,particles:true}};}
export function normalizeMeta(raw){const m=createMeta();if(!raw||raw.version!==VERSION)return m;for(const k of ['credits','runs','wins','bestTime','bestKills','totalKills'])m[k]=Number.isFinite(raw[k])?Math.max(0,Math.floor(raw[k])):0;for(const b of BASE_UPGRADES)m.base[b.id]=Math.min(b.max,Math.max(0,Math.floor(Number(raw.base?.[b.id])||0)));if(Array.isArray(raw.history))m.history=raw.history.slice(0,10).filter(x=>x&&CLASSES.some(c=>c.id===x.classId)&&Number.isFinite(x.time)&&Number.isFinite(x.kills)).map(x=>({classId:x.classId,time:Math.max(0,x.time),kills:Math.max(0,x.kills),won:!!x.won}));m.collection=collectionMeta(raw.collection);if(raw.settings){m.settings.sound=!!raw.settings.sound;m.settings.volume=Math.max(0,Math.min(1,Number(raw.settings.volume)||0));for(const [k,d] of [['musicVolume',.55],['effectsVolume',.8]])m.settings[k]=Number.isFinite(raw.settings[k])?Math.max(0,Math.min(1,raw.settings[k])):d;m.settings.shake=raw.settings.shake!==false;m.settings.particles=raw.settings.particles!==false;}return m;}
