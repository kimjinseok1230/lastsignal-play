import {CLASSES,UPGRADES,EVOLUTIONS,ENEMY_TYPES,RELAY_POSITIONS,DURATION,WORLD,VERSION,xpRequired} from './data.js?v=3';
...
const COLORS={mint:'#b3e4d9',orange:'#ff875e',floor:'#0d181d'};
...
export class Game {
  ...
  runEvent(event){
    if(event.type==='relay')this.toast('중계기 0'+(event.id+1)+' 신호 발견. 지도와 방향 표시를 따라가세요.',false,7);
    ...
    if(event.type==='elite'){this.spawnEnemy('elite',undefined,undefined,{tier:Math.round(event.t/150)});this.toast('변이체 접근 · 처치하면 보급 상자를 얻습니다.',true,5);this.sound.play('boss');}
    if(event.type==='boss'){this.spawnEnemy('sentinel',undefined,undefined,{tier:event.tier});this.toast(event.tier===1?'첫 번째 감시자가 깨어났습니다.':'심층 감시자가 접근합니다.',true,6);this.sound.play('boss');}
    if(event.type==='final'){this.spawnEnemy('final',undefined,undefined,{tier:1});this.toast('최후의 감시자 · THE SILENCER',true,7);this.sound.play('boss');}
    if(event.type==='evac')this.toast(this.relays.every(r=>r.active)&&this.finalDead?'탈출 신호 연결. 중앙의 원 안에서 5초간 버티세요.':'구조선 도착. 중계기 3개와 최후의 감시자를 확인하세요.',false,8);
  }
  ...
  updateEnemy(e,dt){
    ...
    if(e.boss){
      ...
      if(phase&&!e.phase){e.phase=1;this.ring(e.x,e.y,150,COLORS.orange,.8);this.toast(e.type==='final'?'감시자 과부하 · 공격 간격이 짧아집니다.':'감시자가 폭주합니다. 붉은 영역을 피하세요.',true,4);}
    }
  ...
  kill(e){
    ...
    if(e.boss){this.bossKills++;this.spawnLoot('chest',e.x,e.y,1);this.spawnLoot('med',e.x+42,e.y,1);this.collected+=40;this.shake=10;this.sound.play('relay');this.ring(e.x,e.y,250,COLORS.mint,1.2);this.player.hp=Math.min(this.player.maxHp,this.player.hp+25);if(e.type==='final'){this.finalDead=true;this.toast('최후의 감시자 격파. 15:00에 중앙 탈출 지점이 열립니다.',false,7);}else this.toast('감시자 격파 · 생명력 +25 · 보급 상자 투하',false,5);}
    else if(e.type==='elite'){this.eliteKills++;this.collected+=15;this.spawnLoot('chest',e.x,e.y,1);this.spawnLoot('med',e.x+35,e.y,1);this.toast('변이체 제거 · 보급 상자를 회수하세요.',false,4);}
  ...
  updateLoot(dt){const p=this.player;let xpGain=0;for(const l of this.loot){l.age+=dt;const dx=p.x-l.x,dy=p.y-l.y,d=Math.hypot(dx,dy);if(d<this.stats.pickup||(l.type==='chest'&&d<65))l.magnet=true;if(l.magnet){const speed=clamp(350+l.age*25,350,950);const amount=Math.min(d,speed*dt);if(d){l.x+=dx/d*amount;l.y+=dy/d*amount;}}
      if(d<20){l.dead=true;if(l.type==='xp'){xpGain+=l.value;}
        ...
        else if(l.type==='magnet'){for(const gem of this.loot)if(gem.type==='xp'||gem.type==='credit')gem.magnet=true;this.toast('자기장 활성화 · 모든 신호 파편 수집',false,3);}
        else if(l.type==='chest'){this.pendingLevels++;this.collected+=12;p.hp=Math.min(p.maxHp,p.hp+15);this.toast('보급 회수 · 무료 강화 선택 · 생명력 +15',false,3);}}
  ...
  updateRelays(dt){const p=this.player;let charging=null;for(const r of this.relays){if(r.active||this.t<r.unlock)continue;const d=Math.hypot(p.x-r.x,p.y-r.y);if(d<107){r.charge=Math.min(12,r.charge+dt);charging=r;if(r.charge>=12){r.active=true;this.collected+=35;p.hp=Math.min(p.maxHp,p.hp+35+(this.base.recovery||0)*6);this.pendingLevels++;this.ring(r.x,r.y,400,COLORS.mint,1.2);this.sound.play('relay');this.toast('중계기 0'+(r.id+1)+' 복구 · 무료 강화 · 생명력 회복',false,5);for(const e of this.enemies)if(!e.boss&&Math.hypot(e.x-r.x,e.y-r.y)<400)this.damage(e,350*this.stats.power,'relay');}}}this.charging=charging&&!charging.active?charging:null;}
  getChoices(exclude=[]){
    ...
    const fallback=[{id:'heal',name:'응급 수복',icon:'✚',desc:'생명력을 50% 회복합니다.',type:'보급',level:0},{id:'cache',name:'자원 확보',icon:'◇',desc:'기지 정비용 부품 35개를 확보합니다.',type:'보급',level:0},{id:'overload',name:'한계 돌파',icon:'↗',desc:'모든 피해량이 추가로 5% 증가합니다.',type:'보급',level:0}];for(const u of fallback)if(selected.length<3)selected.push(u);
    return selected;
  }
  ...
  drawDrone(c,x,y,a){c.save();c.translate(x,y);c.rotate(a);c.fillStyle='#a7d9c8';c.fillRect(-8,-5,16,10);c.fillStyle='#264947';c.fillRect(-4,-3,8,6);c.fillStyle='#d4f4d8';c.fillRect(-3,-2,6,4);c.strokeStyle='#93b8a466';c.lineWidth=1;c.strokeRect(-11,-8,22,16);c.restore();}
  ...
  drawEvac(c){if(this.t<DURATION)return;const ready=this.finalDead&&this.relays.every(r=>r.active);c.strokeStyle=ready?COLORS.mint:'#ba9470';c.lineWidth=3;c.setLineDash([15,7]);c.beginPath();c.arc(0,0,125,0,TAU);c.stroke();c.setLineDash([]);if(ready){c.fillStyle='#b3e4d912';c.fill();c.beginPath();c.arc(0,0,135,-Math.PI/2,-Math.PI/2+TAU*this.evacuating/5);c.stroke();}c.fillStyle=ready?COLORS.mint:'#ba9470';c.font='14px sans-serif';c.textAlign='center';c.fillText(ready?'탈출 신호 · 5초간 머무르세요':'중계기와 최후의 감시자 확인',0,-151);}
  drawDirections(c){
    const targets=this.relays.filter(r=>!r.active&&this.t>=r.unlock).map(r=>({...r,label:'0'+(r.id+1),color:COLORS.orange}));if(this.t>=DURATION)targets.push({x:0,y:0,label:'탈출',color:COLORS.mint});
