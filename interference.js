import {aiming} from './aim.js?v=cat1';
import {Game} from './game.js?v=cat1';
import {WORLD,DURATION,EDITION,ENEMY_TYPES,UPGRADES,EVOLUTIONS,WEAPONS,PROTOCOLS} from './data.js?v=cat1';
import {signalVisuals} from './signal-visuals.js?v=cat1';
...
export class SignalGame extends Game {
  ...
  updateWeapons(dt){
    ...
    if(this.u.arc&&p.arcCd<=0){let target=this.aimTarget(470,1.1);if(target){p.arcCd=(2.7-this.u.arc*.16)/s.haste;const hit=new Set();let from=p;for(let i=0;i<2+this.u.arc+(this.evolved.storm?4:0)&&target;i++){hit.add(target.id);this.beams.push({x:from.x,y:from.y,tx:target.x,ty:target.y,life:.2,max:.2,color:'#3876bc'});this.damage(target,(24+this.u.arc*12)*s.power*(this.evolved.storm?1.35:1)*(this.protocol==='capacitor'?1.2:1),'arc');from=target;target=this.nearest(from,230,hit);}this.sound.play('arc');}}
