import {aiming} from './aim.js?v=3';
import {Game} from './game.js?v=3';
import {WORLD,DURATION,EDITION,ENEMY_TYPES,UPGRADES,EVOLUTIONS,WEAPONS,PROTOCOLS} from './data.js?v=3';
import {signalVisuals} from './signal-visuals.js?v=3';
...
export class SignalGame extends Game {
  constructor(...args){super(...args);this.initSignal();if(typeof Image!=='undefined'){this.groundImage=new Image();this.groundImage.src=new URL('./assets/signal-floor.png',import.meta.url).href;}}
  initSignal(){this.edition=EDITION;this.resonance=25;this.overdrive=0;this.overdriveCount=0;this.combo=0;this.comboTimer=0;this.bestCombo=0;this.perfectDodges=0;this.lastDamageAt=-99;this.nextInterference=48;this.interferenceIndex=0;this.interferenceUntil=0;this.interferenceName='정상 수신';this.lines=[];this.waves=[];this.traps=[];this.wells=[];this.scars=[];this.protocol=null;this.weaponSlots=3;this.shieldTick=0;this.hitStop=0;this.glitch=0;this.railTimer=1.5;this.mineTimer=2;this.relayChallenge={};this.choiceIds=[];this.runDamage={};}
  ...
  activateOverdrive(){if(this.state!=='playing'||this.resonance<100||this.overdrive>0)return false;this.resonance=0;this.overdrive=7;this.overdriveCount++;this.recalculate();this.glitch=.7;this.shake=7;this.ring(this.player.x,this.player.y,300,'#d8ff65',.7);this.sound.play('overdrive');this.toast('과부하 · 7초간 출력·연사 +35% / 받는 피해 +30%',true,4);return true;}
  dash(){const p=this.player;if(this.state!=='playing'||p.dashCd>0)return false;
    ...
    const ok=super.dash();if(ok){this.fx.push({kind:'slash',x:p.x,y:p.y,a:Math.atan2(p.dashY,p.dashX),r:120,color:'#d7ff76',life:.24,max:.24});if(danger){this.perfectDodges++;p.hp=Math.min(p.maxHp,p.hp+3);this.resonance=Math.min(100,this.resonance+18);p.dashCd*=.7;this.float(p.x,p.y-47,'PERFECT +3','#d8ff65',16);this.sound.play('perfect');}if(this.classId==='runner'){this.traps.push({x:p.x,y:p.y,age:0,life:1.1,arm:.6,damage:45*this.stats.power,r:88,echo:true});}}return ok;
  }
  ...
  triggerInterference(){
    ...
    if(type===0){this.interferenceName='절단 교차';this.toast('간섭 / 절단 교차 · 붉은 사격선에서 벗어나세요.',true,4);const a=this.random()*TAU,nx=Math.cos(a),ny=Math.sin(a),tx=-ny,ty=nx;for(let i=-1;i<=1;i++){const cx=p.x+tx*i*130,cy=p.y+ty*i*130;this.addLine(cx-nx*650,cy-ny*650,cx+nx*650,cy+ny*650,1.6+Math.abs(i)*.35,.36,16,25+this.t*.015);}}
    else if(type===1){this.interferenceName='추적 군집';this.toast('간섭 / 추적 군집 · 측면의 사냥꾼을 경계하세요.',true,4);for(let i=0;i<8+Math.floor(this.t/180);i++){const a=i*TAU/10+this.t;this.spawnEnemy('wraith',clamp(p.x+Math.cos(a)*580,-WORLD+40,WORLD-40),clamp(p.y+Math.sin(a)*580,-WORLD+40,WORLD-40));}if(this.t>160)this.spawnEnemy('sniper');}
    else{this.interferenceName='역위상 수축';this.toast('간섭 / 역위상 수축 · 파동 틈으로 이동하거나 대시하세요.',true,5);this.addWave(p.x,p.y,430,this.random()*TAU,1.2);}
  }
  ...
  updateEnemy(e,dt){if(e.dead)return;e.exposed=Math.max(0,(e.exposed||0)-dt);e.chill=Math.max(0,(e.chill||0)-dt);e.burn=Math.max(0,(e.burn||0)-dt);e.burnTick=(e.burnTick??.5)-dt;
    ...
    if(e.boss){
      const stage=e.hp/e.maxHp<.33?2:e.hp/e.maxHp<.66?1:0;if(stage>e.bossStage){e.bossStage=stage;this.glitch=.35;this.shake=8;this.sound.play('boss');this.toast('감시자 / PHASE 0'+(stage+1)+' · 패턴 가속',true,4);}
      ...
      if(e.patternCd<=0){e.patternCd=9-stage*1.2;e.exposed=4.5;const pattern=(Math.floor(this.t/9)+stage)%3;if(pattern===0){for(let i=-1;i<=1;i++){const a=e.angle+i*.36;this.addLine(e.x,e.y,e.x+Math.cos(a)*1100,e.y+Math.sin(a)*1100,1.5+Math.abs(i)*.18,.35,18,e.damage);}}
        ...
        else{for(let i=0;i<4+stage;i++){const a=i*TAU/(4+stage);this.hazards.push({x:p.x+Math.cos(a)*115,y:p.y+Math.sin(a)*115,r:68,life:1.5,max:1.5,damage:e.damage});}}this.float(e.x,e.y-e.r-32,'CORE EXPOSED','#d7ff71',13);}
    }
  }
  ...
  updateWeapons(dt){
    ...
    if(this.u.arc&&p.arcCd<=0){let target=this.aimTarget(470,1.1);if(target){p.arcCd=(2.7-this.u.arc*.16)/s.haste;const hit=new Set();let from=p;for(let i=0;i<2+this.u.arc+(this.evolved.storm?4:0)&&target;i++){hit.add(target.id);this.beams.push({x:from.x,y:from.y,tx:target.x,ty:target.y,life:.2,max:.2,color:'#8aedff'});this.damage(target,(24+this.u.arc*12)*s.power*(this.evolved.storm?1.35:1)*(this.protocol==='capacitor'?1.2:1),'arc');from=target;target=this.nearest(from,230,hit);}this.sound.play('arc');}}
  ...
  fireMain(target){super.fireMain(target);this.fx.push({kind:'muzzle',x:this.player.x,y:this.player.y,a:this.player.angle,life:.07,max:.07,color:this.overdrive>0?'#dcff66':'#daf4d4'});}
  ...
  kill(e){if(e.dead)return;super.kill(e);this.combo++;this.comboTimer=3.2;this.bestCombo=Math.max(this.bestCombo,this.combo);this.resonance=Math.min(100,this.resonance+(e.boss?22:e.type==='elite'?12:.9));this.scars.push({x:e.x,y:e.y,r:e.r,color:e.color,life:e.boss?7:3.5});if(e.boss){this.hitStop=.09;this.glitch=.7;this.fx.push({kind:'burst',x:e.x,y:e.y,r:260,life:1,max:1,color:'#e0ffac'});}else if(this.combo%25===0){this.float(this.player.x,this.player.y-54,this.combo+' CHAIN','#d8ff6a',15);this.resonance=Math.min(100,this.resonance+8);}}
  ...
  getChoices(exclude=[]){if(this.level>=8&&!this.protocol)return PROTOCOLS.map(p=>({...p,level:0}));const owned=WEAPONS.filter(id=>this.u[id]);const eligible=UPGRADES.filter(u=>(this.u[u.id]||0)<u.max&&(!WEAPONS.includes(u.id)||this.u[u.id]||owned.length<this.weaponSlots));const evos=EVOLUTIONS.filter(e=>!this.evolved[e.id]&&(this.u[e.weapon]||0)>=5&&(this.u[e.support]||0)>=2);const result=[];
    ...
    for(const u of [{id:'heal',name:'응급 수복',icon:'✚',desc:'생명력 50% 회복',type:'보급',level:0},{id:'cache',name:'자원 확보',icon:'◇',desc:'기지 부품 35개 획득',type:'보급',level:0},{id:'overload',name:'한계 돌파',icon:'↗',desc:'모든 피해량 +5%',type:'보급',level:0}])if(result.length<3)result.push(u);return result;
  }
  applyUpgrade(id){if(PROTOCOLS.some(p=>p.id===id)){const hp=this.player.hp,max=this.player.maxHp;this.protocol=id;this.recalculate();if(this.player.maxHp>max)this.player.hp=Math.min(this.player.maxHp,hp+(this.player.maxHp-max));this.toast(PROTOCOLS.find(p=>p.id===id).name+' / 신호 변조 적용',false,4);return;}super.applyUpgrade(id);if(EVOLUTIONS.some(e=>e.id===id)){this.hitStop=.06;this.glitch=.5;this.sound.play('evolution');}}
